...
// The RateLimitOptionsFunc type is an adapter to allow the use of ordinary
// function as RateLimitOptions mutator.
type RateLimitOptionsFunc func(context.Context, *gen.RateLimitOptionsMutation) (gen.Value, error)

// Mutate calls f(ctx, m).
func (f RateLimitOptionsFunc) Mutate(ctx context.Context, m gen.Mutation) (gen.Value, error) {
	if mv, ok := m.(*gen.RateLimitOptionsMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *gen.RateLimitOptionsMutation", m)
}
