...
import (
	...
	redisLib "github.com/isikhi/go-rate-limiter/third_party/redis"
	"github.com/isikhi/go-rate-limiter/third_party/validate"
)
...
func (s *Server) setGlobalMiddleware() {
	...
	s.router.Use(middleware.Metric)
...
func (s *Server) Run() {
	...
	fmt.Println(`Server going to bootstrap.`)
	go func() {
		httpServerStart(s)
	}()
	go func() {
...
func grpcServerStart(s *Server) {
	lis, err := net.Listen("tcp", fmt.Sprintf(":%d", s.cfg.Api.RPCPort))
