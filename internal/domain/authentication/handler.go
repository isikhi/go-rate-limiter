...
import (
	...
	"github.com/isikhi/go-rate-limiter/internal/utility/param"
	"github.com/isikhi/go-rate-limiter/internal/utility/request"
...
func generateJWTToken(user *gen.User) (string, error) {
	...
	ss, err := token.SignedString([]byte(cfg.Api.JWTSecret))
	return ss, err
}
...
func (h *Handler) Login(w http.ResponseWriter, r *http.Request) {
	...
	case "jwt":
		...
		respond.Json(w, http.StatusOK, map[string]string{"access_token": token, "type": "Bearer"})
	case "session":
		h.session.Put(ctx, string(middleware.KeyID), user.ID)
	default:
		fmt.Println("No authentication method specified")
		respond.Error(w, http.StatusInternalServerError, errors.New("authentication server is not good"))
		return
	}
