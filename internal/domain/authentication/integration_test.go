...
import (
	...
	"github.com/isikhi/go-rate-limiter/internal/utility/csrf"
	_ "github.com/lib/pq"
	...
	"github.com/isikhi/go-rate-limiter/ent/gen"
	"github.com/isikhi/go-rate-limiter/internal/middleware"
	"github.com/isikhi/go-rate-limiter/third_party/postgresstore"
)
...
func TestHandler_RegisterIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			RegisterHTTPEndPoints(router, session, repo, cfg)
...
func TestHandler_ProtectedIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rr = httptest.NewRequest(http.MethodGet, "/api/v1/private", nil)
			ww = httptest.NewRecorder()
...
func TestHandler_MeIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rr = httptest.NewRequest(http.MethodGet, "/api/v1/private/me", nil)
			ww = httptest.NewRecorder()
...
func TestHandler_Force_LogoutIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Now we log in as super admin to log out test@mail.com
			admin := &LoginRequest{
			...
			rr = httptest.NewRequest(http.MethodPost, "/api/v1/private/logout/2", nil)
			ww = httptest.NewRecorder()
			rr.AddCookie(&http.Cookie{
				Name:  sessionName,
				Value: adminToken,
			...
			// Check normal user ID 2 cannot access private route anymore
			rr = httptest.NewRequest(http.MethodGet, "/api/v1/private", nil)
			ww = httptest.NewRecorder()
			rr.AddCookie(&http.Cookie{
				Name:  sessionName,
...
func TestHandler_Csrf_Valid_TokenIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rr = httptest.NewRequest(http.MethodGet, "/api/v1/private/csrf", nil)
			ww = httptest.NewRecorder()
